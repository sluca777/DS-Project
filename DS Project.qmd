...
The dimensions of the raw childcare costs data before merging are 34567 by 61, while the dimensions of the counties table are 3144 by 4. After the merge, the dimensions of the cleaned data are 23342 by 64.
...
descstats <- cleaned_data |>
  select(mcsa, mfccsa) |>
  summarise_all(list(mean = mean, median = median, IQR = IQR), na.rm = TRUE)

datatable(descstats,
          caption = "Table 1: Descriptive Stats for Childcare Costs",
          colnames = c("Number", "Center-Based Mean", "Family-Based Mean", "Center-Based Median", "Family-Based Median", "Center-Based IQR", "Family-Based IQR")
...
datatable(countycosts,
          caption = "Table 2: County Costs",
          colnames = c("County Name", "Childcare Average", "Family Average")
...
datatable(
  state_costs,
  caption = "Table 3: Costs by State",
  colnames = c("Number", "State", "Center-Based", "Infant", "Toddler", "Preschooler")
)
```
...
race_weighted_costs <- cleaned_data |>
  summarize(
    ...
    native_infant = weighted.mean(mc_infant, one_race_i, na.rm = TRUE),
    asian_infant = weighted.mean(mc_infant, one_race_a, na.rm = TRUE),
    islander_infant = weighted.mean(mc_infant, one_race_h, na.rm = TRUE),
    ...
    native_toddler = weighted.mean(mc_toddler, one_race_i, na.rm = TRUE),
    asian_toddler = weighted.mean(mc_toddler, one_race_a, na.rm = TRUE),
    islander_toddler = weighted.mean(mc_toddler, one_race_h, na.rm = TRUE),
    ...
    native_preschool = weighted.mean(mc_preschool, one_race_i, na.rm = TRUE),
    asian_preschool = weighted.mean(mc_preschool, one_race_a, na.rm = TRUE),
    islander_preschool = weighted.mean(mc_preschool, one_race_h, na.rm = TRUE),
...
datatable(
  ...
  caption = "Table 4: Costs by Race",
  colnames = c("Childcare Type", "White", "Black", "Native American", "Asian", "Pacific Islander", "Other")
)
...
datatable (care_cost_comparison,
           caption = "Table 5: Comparison of Center-Based and Family Costs",
           colnames = c("Average Center Infant", "Average Family Infant", "Average Center Toddler", "Average Family Toddler", "Average Center Preschool", "Average Family Preschool")
...
lowpoverty <- cleaned_data |>
  select(pr_f, mc_infant, mc_toddler, mc_preschool, mfcc_infant, mfcc_toddler, mfcc_preschool) |>
  arrange(pr_f) |>
  slice_head(n = 5)

datatable(lowpoverty,
          caption = "Table 6: Lowest Poverty Rates",
          colnames = c("Poverty Rate", "Center-Based Infant", "Center-Based Toddler", "Center-Based Preschool", "Family-Based Infant", "Family-Based Toddler", "Family-Based Preschool"
          )
  
)

#Highest 5 Poverty Rates
highpoverty <- cleaned_data |>
  select(pr_f, mc_infant, mc_toddler, mc_preschool, mfcc_infant, mfcc_toddler, mfcc_preschool) |>
  arrange(desc(pr_f)) |>
  slice_head(n = 5)

datatable(highpoverty,
          caption = "Table 7: Highest Poverty Rates",
          colnames = c("Poverty Rate", "Center-Based Infant", "Center-Based Toddler", "Center-Based Preschool", "Family-Based Infant", "Family-Based Toddler", "Family-Based Preschool"
                       ))

```

Childcare Costs in Counties with High Service Sector Employment Determine if counties with a higher proportion of service sector employment see different childcare costs.
...
datatable (
  service,
  caption = "Table 8: Costs in High Service Sector Employment Counties",
  colnames = c("Number", "Median Infant Cost", "Median Cost Toddler", "Median Cost Preschool")
)
...
ggplot(cost_trends, aes(x = study_year)) +
  ...
  labs(title = "Figure 1: Center-Based Childcare Costs Trends from 2008 to 2018",
       x = "Year",
       y = "Average Cost",
       color = "Age Group")
```
...
ggplot(cleaned_data, aes(x = mhi_2018, y = mcsa)) +
  ...
  labs(title = "Figure 2: Scatterplot of Center-Based Childcare Costs vs. Median Household Income",
...
ggplot(cleaned_data, aes(x = mhi_2018, y = mfccsa)) +
  geom_point(aes(color = county_fips_code)) +
  labs(title = "Figure 3: Scatterplot of Family-Based Childcare Costs vs. Median Household Income",
       x = "Median Household Income ($)",
...
ggplot(cleaned_data, aes(x = mcsa)) +
  ...
  labs(title = "Figure 4: Center-Based Costs Distribution",
...
ggplot(cleaned_data, aes(x = mfccsa)) +
  geom_histogram(bins = 30, fill = "blue") +
  labs(title = "Figure 5: Family Costs Distribution",
       x = "Family Costs ($/week)",
       y = "Frequency") +
  theme_minimal()
...
ggplot(cleaned_data, aes(x = mhi_2018)) +
  geom_boxplot() +
  labs(title = "Figure 6: Median Household Income",
       x = "Income") +
  theme_minimal()
```
...
Impact of Recent Economic Changes: Research could also explore the impact of recent global economic disruptions, like the COVID-19 pandemic, on childcare costs and family economics to inform current policy discussions. Qualitative Studies: Incorporating qualitative research could help capture the personal experiences and challenges families face regarding childcare, offering a more holistic view of the issue's impact on American family life.
